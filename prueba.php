...
SELECT
    ...
    CONCAT(RTRIM(Prop.nombres), ' ', RTRIM(Prop.apellidos)) AS COMNOMBREPROP,
    ...
    CONCAT(RTRIM(Test.nombres), ' ', RTRIM(Test.apellidos)) AS COMNOMBRETESTI,
    REPLACE(Test.direccion, ',', ' ') AS COMDIRECRESTESTI,
    Test.telefono AS COMTELETESTIGO,
    ValorCompSMLV(Comp.Tcomparendos_ID) AS COMVALOR,
    (SELECT Tparameconomicos_vadicComp FROM parametros_economicos LIMIT 1) AS COMVALAD,
    (SELECT divipo FROM sedes LIMIT 1) AS COMORGANISMO,
