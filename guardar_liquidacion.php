...
if ($tipoTramite == 4) {
    ...
    foreach ($tramitesSeleccionados as $comparendos) {
        ...
        sqlsrv_query($mysqli, $consultaDetalle, array(), array('Scrollable' => 'buffered'));
    ...
    foreach ($tramitesSeleccionados as $dt) {
        ...
        $resultado_dt = sqlsrv_query($mysqli, $consulta_dt, array(), array(
            'Scrollable' => 'buffered'
        ));
        ...
        // Ejecutar la consulta
        if (sqlsrv_query($mysqli, $consultaDetalle, array(), array(
            'Scrollable' => 'buffered'
        )) !== TRUE) {
        ...
        $result_primer = sqlsrv_query($mysqli, $sql_primer, array(), array(
            'Scrollable' => 'buffered'
        ));
    ...
    foreach ($tramitesSeleccionadosArray as $ap) {
        ...
        $resultado_ap = sqlsrv_query($mysqli, $consulta_ap, array(), array(
            'Scrollable' => 'buffered'
        ));
    ...
    // Realizar la inserción en la tabla liquidaciones
    $consulta = "SET NOCOUNT ON;INSERT INTO liquidaciones (tipo_tramite, ciudadano, placa, tipo_servicio, clase_vehiculo, clasificacion_vehiculo, usuario, fecha, fechayhora, empresa,nota_credito,comparendo)
               VALUES ('$tipoTramite', '$ciudadano', '$placa', '$tipoServicio', '$claseVehiculo', '$clasificacionVehiculo', '$idusuario', '$fecha', '$fechayhora', '$empresa','$nota_credito',0);SELECT scope_identity() as lastid";
    ...
    foreach ($tramitesSeleccionados as $tramiteId) {
        // Realizar la inserción en la tabla detalle_liquidaciones
        $consultaDetalle = "INSERT INTO detalle_liquidaciones (liquidacion, tramite) VALUES ('$liquidacionId', '$tramiteId')";

        // Ejecutar la consulta
        $resultado_consultaDetalle = sqlsrv_query($mysqli, $consultaDetalle, array(), array('Scrollable' => 'buffered'));
        ...
        while ($row_detalle_tramite = sqlsrv_fetch_array($resultado_detalle_tramite, SQLSRV_FETCH_ASSOC)) {
			...
			if (sqlsrv_num_rows($resultado_concepto)>0){
				...
				if ($row_concepto['id'] > 0 && $fecha >= $fecha_vigencia_inicial && $fecha <= $rango && !in_array($row_concepto['id'], $repetidos)) {
					...
					if ($row_concepto['valor_modificacble'] == 1) {
					...
					VALUES ('$liquidacionId', '$tramiteId', '" . $row_detalle_tramite['concepto_id'] . "',".ROUND($valor).",'" . $row_concepto['terceros'] . "')";
...
if ($nota_credito > 0) { // Si se uso nota credito se guarda en notas_credito_usadaas y se actualiza el saldo
    ...
    $resultado_nc = sqlsrv_query($mysqli, $consulta_nc, array(), array(
        'Scrollable' => 'buffered'
    ));
    ...
    if ($row_nc['saldo'] > $nota_credito) {
        ...
        $resultadoUpdate = sqlsrv_query($mysqli, $queryUpdate, array(), array(
            'Scrollable' => 'buffered'
        ));
    ...
    sqlsrv_query($mysqli, $consulta, array(), array(
        'Scrollable' => 'buffered'
    ));
